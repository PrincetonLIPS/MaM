...
from utils import multiple_selfies_to_int, multiple_smiles_to_int, get_alphabet_from_smiles
...
def load_molecules_from_smiles(smiles_list, alphabet, largest_len, option):
    ...
    data = torch.tensor(data, dtype=torch.long) # {0,1,..,K-1} --> {1,2,..,K}
...
def load_moses(config):
    ...
    data_folder_path = os.path.join(config.data_dir, 'moses')
    if not os.path.exists(data_folder_path):
        os.makedirs(data_folder_path)
    moses_alphabet_path = os.path.join(data_folder_path, f'moses_alphabet_{config.string_type}.json')
    ...
    if config.load_full:
        ...
        train_smiles_list = all_smiles_list[:2000]
        val_smiles_list = all_smiles_list[2000:3000]  
    dataset_train = load_molecules_from_smiles(train_smiles_list, alphabet, largest_len, config.string_type)
    dataset_val = load_molecules_from_smiles(val_smiles_list, alphabet, largest_len, config.string_type)
    dataset_test = load_molecules_from_smiles(test_smiles_list, alphabet, largest_len, config.string_type)
